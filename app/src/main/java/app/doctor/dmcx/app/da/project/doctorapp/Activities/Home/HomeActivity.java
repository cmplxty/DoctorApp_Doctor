...
import app.doctor.dmcx.app.da.project.doctorapp.Fragments.AppPreferenceFragmentManager;
import app.doctor.dmcx.app.da.project.doctorapp.Fragments.Home.PrefsFragment;
import app.doctor.dmcx.app.da.project.doctorapp.Fragments.Home.AssistantFragment;
...
import app.doctor.dmcx.app.da.project.doctorapp.Fragments.Home.AudioCallHistoryFragment;
...
import app.doctor.dmcx.app.da.project.doctorapp.Seed.ISeeder;
import app.doctor.dmcx.app.da.project.doctorapp.Seed.SeedController;
...
public class HomeActivity extends BaseActivity implements INavHeader, SinchService.StartFailedListener, ISeeder {
        ...
        public void run() {
            switch (item.getItemId()) {
                ...
                case R.id.audioCallDMI:
                    loadNavFragment(getString(R.string.audio_call), new AudioCallHistoryFragment(), FragmentNames.AudioCall);
                    break;
                case R.id.blogDMI:
                    loadNavFragment(getString(R.string.blog), new BlogFragment(), FragmentNames.Blog);
                    break;
                case R.id.assistantDMI:
                    loadNavFragment(getString(R.string.assistant), new AssistantFragment(), FragmentNames.Assistant);
                    break;
                case R.id.prefernceDMI:
                    loadNavPreferenceFragment();
                    break;
                case R.id.signOutDMI:
    ...
    private static class SinchServiceHandler extends Handler {
        @Override
    ...
    private void init() {
        ...
        // Service
        sinchServiceHandler.postDelayed(new SinchServiceRunnable(), 1500);
    }
    ...
    private void onStartMethod() {
        ...
        AppSupportFragmentManager.replace(RefActivity.refACActivity.get(), AppSupportFragmentManager.homeFragmentContainer, new DashboardFragment(), FragmentNames.Dashboard);
    ...
    private void loadBottomNavFragment(String title, Fragment fragment, String tag, int id) {
        ...
        AppSupportFragmentManager.replace(RefActivity.refACActivity.get(), AppSupportFragmentManager.homeFragmentContainer, fragment, tag);
    ...
    private void loadNavPreferenceFragment() {
        toolbar.setTitle(R.string.preference);
        AppPreferenceFragmentManager.repace(RefActivity.refACActivity.get(), new PrefsFragment());
    }
    // Methods
    ...
    protected void onDestroy() {
        if (getSinchServiceBinder() != null) {
            getSinchServiceBinder().stopClient();
        }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        if (Vars.currentFragment != null) {
            ...
            } else if (Objects.equals(Vars.currentFragment.getTag(), FragmentNames.Profile)) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.editProfileMIT: {
                ...
                AppSupportFragmentManager.replace(RefActivity.refACActivity.get(), AppSupportFragmentManager.homeFragmentContainer, new ProfileEditFragment(), FragmentNames.ProfileEdit, bundle);
                break;
            }
            case R.id.logoutMIT:
                ProfileController.SignOut();
    ...
    public void onBackPressed() {
        if (drawer_layout.isDrawerOpen(GravityCompat.START)) {
            ...
            if (AppPreferenceFragmentManager.currentFragment(RefActivity.refACActivity.get()) != null) {
                if (AppPreferenceFragmentManager.currentFragment(RefActivity.refACActivity.get()).getTag().equals(AppPreferenceFragmentManager.PREFERENCE_TAG)) {
                    AppPreferenceFragmentManager.remove(RefActivity.refACActivity.get());
                    onStartMethod();
                }
            } else {
                super.onBackPressed();
